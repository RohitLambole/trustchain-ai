import type { RiskPolicy, RiskPolicyCondition } from "../../risk-policies/domain/risk-policy.types";
import type { PolicyMatch, RiskEvaluationInput } from "../domain/risk-engine.types";

export class PolicyEngine {
  evaluate(input: RiskEvaluationInput, policies: RiskPolicy[]): PolicyMatch[] {
  ...
  builtInMatches(input: RiskEvaluationInput): PolicyMatch[] {
  ...
  private buildFacts(input: RiskEvaluationInput): Record<string, unknown> {
    return {
  ...
  private matches(condition: RiskPolicyCondition, facts: Record<string, unknown>) {
  ...
  private match(name: string, action: PolicyMatch["action"], severity: PolicyMatch["severity"], weight: number, priority: number, reason: string): PolicyMatch {
